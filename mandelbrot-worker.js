...
function countIterationsHP(/* Uint32Array */ x, /* Uint32Array */ y) {
  ...
  while (count < maxIterations) {
    ...
    if ((work1[0] & 0xfff8) != 0 && (work1[0] & 0xfff8) != 0xfff0) break;
...
function multiply(/* int[] */ x, /* int[] */ y, /* int */ count) {
  // Can't allow x == y !
  var neg1 = (x[0] & 0x8000) != 0;
  if (neg1) negate(x, count);
  var neg2 = (y[0] & 0x8000) != 0;
  if (neg2) negate(y, count);
  if (x[0] == 0) {
    for (var i = 0; i < count; i++) work3[i] = 0;
  } else {
    var carry = 0;
    for (var i = count - 1; i >= 0; i--) {
